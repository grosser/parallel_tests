...
namespace :parallel do
  ...
  task :create, :count do |t,args|
    ParallelTests::Tasks.run_in_parallel("rake db:create:all_test RAILS_ENV=#{rails_env}", args.to_hash.merge(:non_parallel => true))
  end

  desc "drop test databases via db:drop --> parallel:drop[num_cpus]"
  task :drop, :count do |t,args|
    ParallelTests::Tasks.run_in_parallel("rake db:drop:all_test RAILS_ENV=#{rails_env}", args.to_hash.merge(:non_parallel => true))
  end
  ...
  desc "recreate test databases via db:structure:load --> parallel:load_structure[num_cpus]"
  task :load_structure, :count do |t,args|
    ParallelTests::Tasks.run_in_parallel("rake db:structure:load RAILS_ENV=#{rails_env}", args)
  end

  desc "recreate multiple test database schemas in parallel"
  task :load_schemas, :count do |t,args|
    ParallelTests::Tasks.run_in_parallel("rake db:test:schema:load RAILS_ENV=#{rails_env}", args)
  end

  desc "load multiple test database fixtures in parallel"
