...
describe ParallelTests::Spinach do
  ...
  describe :run_tests do
    before do
      ...
      File.stub!(:file?).with('script/spinach').and_return true
    end

    def call(*args)
      ParallelTests::Spinach::Runner.run_tests(*args)
    end
    ...
    it "runs bundle exec spinach when on bundler 0.9" do
      ParallelTests.stub!(:bundler_enabled?).and_return true
      should_run_with %r{bundle exec spinach}
      call(['xxx'],1,22,{})
    end

    it "runs script/spinach when script/spinach is found" do
      should_run_with %r{script/spinach}
      call(['xxx'],1,22,{})
    end

    it "runs spinach by default" do
      File.stub!(:file?).with('script/spinach').and_return false
      should_run_with %r{^spinach}
      call(['xxx'],1,22,{})
    end

    it "uses bin/spinach when present" do
      File.stub(:exists?).with("bin/spinach").and_return true
      should_run_with %r{bin/spinach}
      call(['xxx'],1,22,{})
    end

    it "uses options passed in" do
      should_run_with %r{script/spinach .* -p default}
    ...
    context "with parallel profile in config/spinach.yml" do
      before do
        file_contents = 'parallel: -f progress'
        Dir.stub(:glob).and_return ['config/spinach.yml']
        File.stub(:read).with('config/spinach.yml').and_return file_contents
      end

      it "uses parallel profile" do
        should_run_with %r{script/spinach .* foo bar --profile parallel xxx}
        call(['xxx'],1,22, :test_options => 'foo bar')
      end

      it "uses given profile via --profile" do
        should_run_with %r{script/spinach .* --profile foo xxx$}
        call(['xxx'],1,22, :test_options => '--profile foo')
      end

      it "uses given profile via -p" do
        should_run_with  %r{script/spinach .* -p foo xxx$}
        call(['xxx'],1,22, :test_options => '-p foo')
      end
    end

    it "does not use parallel profile if config/spinach.yml does not contain it" do
      file_contents = 'blob: -f progress'
      should_run_with  %r{script/spinach .* foo bar}
      Dir.should_receive(:glob).and_return ['config/spinach.yml']
      File.should_receive(:read).with('config/spinach.yml').and_return file_contents
      call(['xxx'],1,22,:test_options => 'foo bar')
    end

    it "does not use the parallel profile if config/spinach.yml does not exist" do
      should_run_with %r{script/spinach} # TODO this test looks useless...
  ...
  describe :line_is_result? do
    it "should match lines with only one scenario" do
      ...
      ParallelTests::Spinach::Runner.line_is_result?(line).should be_true
    end
    ...
    it "should not match other lines" do
      ...
      ParallelTests::Spinach::Runner.line_is_result?(line).should be_false
    end
  end
...
EOF
      ParallelTests::Spinach::Runner.find_results(output).should == ["7 scenarios (3 failed, 4 passed)", "33 steps (3 failed, 2 skipped, 28 passed)", "4 scenarios (4 passed)", "40 steps (40 passed)", "1 scenario (1 passed)", "1 step (1 passed)"]
    end
  end

  describe :summarize_results do
    def call(*args)
      ParallelTests::Spinach::Runner.summarize_results(*args)
    end
